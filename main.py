def on_up_pressed():
    if game_mode == 1:
        if camel.y == 165:
            camel.set_position(camel.x, 119)
        elif camel.y == 119:
            camel.set_position(camel.x, 56)
        else:
            pass
    elif False:
        pass
...
def on_down_pressed():
    if game_mode == 1:
        if camel.y == 56:
            ...
            camel.set_position(camel.x, 165)
        else:
            pass
    elif False:
        pass
...
def screen_1():
    global snake, snake2, snake3
    scene.camera_follow_sprite(camel)
    camel.set_stay_in_screen(True)
    camel.set_position(120, 56)
    animation.run_image_animation(camel, assets.animation("""
        CamelGif
    """), 300, True)
    scroller.set_background_scroll_offset(0, -19)
    ...
    snake3 = sprites.create(img("""
        ...
        """),
        SpriteKind.enemy)
    snake.set_position(165, 0)
    snake2.set_position(165, 0)
    snake3.set_position(165, 0)
    scaling.scale_to_pixels(snake, 16, ScaleDirection.UNIFORMLY, ScaleAnchor.MIDDLE)
    scaling.scale_to_pixels(snake2, 16, ScaleDirection.UNIFORMLY, ScaleAnchor.MIDDLE)
    scaling.scale_to_pixels(snake3, 16, ScaleDirection.UNIFORMLY, ScaleAnchor.MIDDLE)
    animation.run_image_animation(snake, assets.animation("""
        myAnim0
    """), 200, True)
    animation.run_image_animation(snake2, assets.animation("""
        myAnim0
    """), 200, True)
    animation.run_image_animation(snake3, assets.animation("""
        myAnim0
    """), 200, True)
    snake.set_stay_in_screen(False)
    snake2.set_stay_in_screen(False)
    snake3.set_stay_in_screen(False)
random = 0
snake3: Sprite = None
snake2: Sprite = None
snake: Sprite = None
...
game_mode = 0
if game_mode == 0:
    scene.center_camera_at(100, 100)
    ...
    mySprite = sprites.create(img("""
        ...
        """),
        SpriteKind.player)
    mySprite.set_velocity(-60, 60)
    mySprite.set_bounce_on_wall(True)
    scaling.scale_to_pixels(mySprite, 50, ScaleDirection.UNIFORMLY, ScaleAnchor.MIDDLE)
    for index in range(4):
        music.play_melody("D E C F E D E C ", 263)
...
def on_forever():
    ...
    if game_mode == 1:
        ...
        while game_mode == 1:
            random = randint(1, 3)
            if random == 1:
                snake.set_position(261, 56)
                snake.vx = -50
                snake2.set_position(261, 165)
                snake2.vx = -35
                snake3.set_position(300, 119)
                snake3.vx = randint(-80, -90)
            elif random == 2:
                snake.set_position(261, 119)
                snake.vx = -50
                snake2.set_position(261, 56)
                snake2.vx = -35
                snake3.set_position(261, 165)
                snake3.vx = randint(-80, -90)
            else:
                snake.set_position(261, 165)
                snake.vx = -50
                snake2.set_position(261, 119)
                snake2.vx = -35
                snake3.set_position(261, 56)
                snake3.vx = randint(-80, -90)
            pause(6000)
forever(on_forever)
